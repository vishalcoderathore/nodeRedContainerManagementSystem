...
local secretPart1 = os.getenv("JWT_SECRET")
local secret = "";

local M = {}

function M.set(secretPart2)
    secret = secretPart1 .. secretPart2
    ngx.log(ngx.INFO, "Secret Key: " .. secret)
...
function M.auth()
    local claim_specs = {};
    claim_specs["exp"]=tostring(os.time());
    ...
    if claim_specs ~= nil then
        ...
        ngx.log(ngx.INFO, "For Claim : " .. claim  ..", Spec should be <= Claim_Value")
	    ngx.log(ngx.INFO, "For Claim : " .. claim  ..", Spec vs Claim_Value (Inside Token): " .. spec.. " <=  " .. claim_value)
           ...
           -- end
            ...
            if not (tonumber(claim_value)>=tonumber(spec)) then
                blocking_claim = claim
                break;
            end
        end
